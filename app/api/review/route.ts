...
export async function GET() {
  try {
    ...
    // Ensure review_sessions has user_id column
    await query(`ALTER TABLE review_sessions ADD COLUMN IF NOT EXISTS user_id UUID`)

    const overdue = await query(`
      SELECT * FROM tasks
      WHERE completed = FALSE AND user_id = $1 AND due_date < CURRENT_DATE
    ...
    const skipped = await query(`
      SELECT * FROM tasks
      WHERE completed = FALSE AND user_id = $1
        AND is_recurring = TRUE AND next_recur_at < NOW()
    ...
    // Increment decay scores
    await query(`
      UPDATE tasks SET decay_score = decay_score + 1
      WHERE completed = FALSE AND user_id = $1
    `, [userId])
...
export async function POST() {
  try {
    ...
    if (!userId) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    await query(`
      INSERT INTO review_sessions (user_id) VALUES ($1)
    `, [userId])
